				...
				<>
					{isChatbotFocus && (
						<div
							...
							onClick={() => setIsChatbotFocus(false)}
						/>
						)}

					<span
						className="fixed bottom-5 right-5 z-50 flex items-center justify-center bg-white p-2 rounded-full shadow-lg border-4 border-yellow-400 outline outline-4 outline-white"
						style={{ width: 72, height: 72 }}
						onClick={() => {
							setIsChatbotFocus(false);
							window.dispatchEvent(new CustomEvent('open-chatbot'));
						}}
					>
						<svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="none" viewBox="0 0 24 24"><path fill="currentColor" d="M12 2C6.48 2 2 6.03 2 11c0 2.39 1.05 4.56 2.83 6.23-.13.48-.51 1.7-.7 2.32-.13.41.28.78.68.64.66-.23 1.98-.7 2.6-.92C9.13 19.76 10.53 20 12 20c5.52 0 10-4.03 10-9s-4.48-9-10-9Zm-3 8.5c.83 0 1.5.67 1.5 1.5S9.83 13.5 9 13.5 7.5 12.83 7.5 12s.67-1.5 1.5-1.5Zm6 0c.83 0 1.5.67 1.5 1.5s-.67 1.5-1.5 1.5-1.5-.67-1.5-1.5.67-1.5 1.5-1.5Zm-6.65 4.15c.2-.2.51-.2.71 0C8.68 15.32 10.27 16 12 16s3.32-.68 3.94-1.35a.5.5 0 1 1 .71.7C15.82 16.18 14.02 17 12 17s-3.82-.82-4.65-1.65a.5.5 0 0 1 0-.7Z"/></svg>
					</span>
				</>
			)}
			<div
			className={`fixed bottom-6 right-6 z-50 p-3 rounded-full bg-white transition-all duration-300 ${
				isChatbotFocus ? "ring-4 ring-purple-500 scale-110" : ""
			}`}
			>
			<ChatbotPopup />
			</div>
